...
async def _(e):
    ...
    await e.reply(
        "`Success!\nYou are all set to use Google Drive with Ultra Userbot.`",
...
async def _(e):
    ...
    async with ultra_bot.asst.conversation(e.sender_id) as conv:
...
async def otvaar(event):
    await event.edit(
        "Other Variables to set for @TheUltra:",
        buttons=[
            [
...
# --------------------------------------------------------------------#
telegraph = Telegraph()
r = telegraph.create_account(short_name="Ultra")
auth_url = r["auth_url"]
# --------------------------------------------------------------------#
