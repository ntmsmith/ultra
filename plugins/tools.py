...
async def _(ult):
    ...
    if not input_str:
        ...
        mentions = "**Bots in **{}: \n".format(input_str)
        try:
            chat = await Ultra_bot.get_entity(input_str)
        except Exception as e:
            await eor(ult, str(e))
            return None
    try:
        async for x in Ultra_bot.iter_participants(
...
async def lastname(steal):
    ...
    try:
        async with Ultra_bot.conversation(chat) as conv:
            try:
