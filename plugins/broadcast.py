...
@Ultra_cmd(pattern="add ?(.*)", allow_sudo=False)
async def broadcast_adder(event):
    ...
    if msgg == "all":
        ...
        chats = [
            e.entity
            for e in await Ultra.get_dialogs()
...
@Ultra_cmd(pattern="rem ?(.*)", allow_sudo=False)
...
@Ultra_cmd(pattern="listchannels")
async def list_all(event):
    ...
    for channel in channels:
        name = ""
        try:
            name = (await Ultra.get_entity(int(channel))).title
        except:
            name = ""
        msg += f"=> **{name}** [`{channel}`]\n"
...
@Ultra_cmd(pattern="forward ?(.*)", allow_sudo=False)
async def forw(event):
    ...
    for channel in channels:
        try:
            await Ultra_bot.forward_messages(int(channel), previous_message)
            sent_count += 1
            await x.edit(
                f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}",
            )
            ...
            try:
                await Ultra_bot.send_message(
                    Var.LOG_CHANNEL, f"Error in sending at {channel}."
                )
                await Ultra_bot.send_message(Var.LOG_CHANNEL, "Error! " + str(error))
    ...
    if error_count > 0:
        try:
            await Ultra_bot.send_message(Var.LOG_CHANNEL, f"{error_count} Errors")
        except BaseException:
            await x.edit("Set up log channel for checking errors.")


@Ultra_cmd(pattern="broadcast ?(.*)", allow_sudo=False)
async def sending(event):
    ...
    if event.reply_to_msg_id:
        ...
        if not previous_message.web_preview and previous_message.photo:
            file = await Ultra_bot.download_file(previous_message.media)
            uploaded_doc = await Ultra_bot.upload_file(file, file_name="img.png")
            raw_text = previous_message.text
            for channel in channels:
                try:
