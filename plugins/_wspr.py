...
async def _(e):
    ...
    if put:
        try:
            results = await Ultra_bot.inline_query(Var.BOT_USERNAME, f"msg {put}")
        except rep:
            return await eor(
                e,
...
async def _(e):
    ...
    if "wspr" not in vvv:
        try:
            logi = await Ultra_bot(gu(id=query))
        ...
        try:
            logi = await Ultra_bot.get_entity(query)
            button = [
