...
async def gcast(event):
    ...
    async for x in Ultra_bot.iter_dialogs():
        if x.is_group:
            chat = x.id
            try:
                done += 1
                await Ultra_bot.send_message(chat, msg)
            except:
                er += 1
...
async def gkick(e):
    ...
    async for gkick in e.client.iter_dialogs():
        if gkick.is_group or gkick.is_channel:
            try:
                await Ultra_bot.kick_participant(gkick.id, userid)
                chats += 1
            except:
                pass
...
@Ultra_bot.on(events.ChatAction)
async def _(e):
