...
async def _(ult):
    ...
    try:
        await Ultra_bot(functions.account.UpdateProfileRequest(about=set))
...
async def _(ult):
    ...
    photo = None
    try:
        photo = await Ultra_bot.download_media(reply_message, TMP_DOWNLOAD_DIRECTORY)
    except Exception as ex:
        await ok.edit("Error occured.\n`{}`".format(str(ex)))
    else:
        if photo:
            await ok.edit("`Uploading it to my profile...`")
            file = await Ultra_bot.upload_file(photo)
            try:
...
async def remove_profilepic(delpfp):
    ...
    await Ultra_bot(DeletePhotosRequest(id=input_photos))
    await ok.edit(f"`Successfully deleted {len(input_photos)} profile picture(s).`")
    await asyncio.sleep(10)
    await ok.delete()


@Ultra_cmd(pattern="gpoto ?(.*)")
async def gpoto(e):
    ult = e.pattern_match.group(1)
    a = await eor(e,"`Processing...`")
    try:
        okla = await Ultra_bot.download_profile_photo(
            ult, "profile.jpg", download_big=True
        )
        await a.delete()
        await Ultra_bot.send_message(e.chat_id, file=okla)
        os.remove(okla)
