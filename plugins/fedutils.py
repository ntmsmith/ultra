...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            downloaded_file_name = await Ultra_bot.download_media(
    ...
    try:
        if str(FBAN) in DEVLIST:
            ...
            try:
                x = await Ultra_bot(GetFullUserRequest(FBAN))
                uid = x.user.id
    ...
    if not len(fedList):
        for a in range(3):
            async with Ultra_bot.conversation("@MissRose_bot") as bot_conv:
    ...
    try:
        await Ultra_bot.send_message(chat, f"/start")
    except BaseException:
    ...
    for fed in fedList:
        if udB.get("EXCLUDE_FED") and fed in excludeFed:
            await Ultra_bot.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
        await Ultra_bot.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await Ultra_bot.send_message(chat, f"/fban {FBAN} {REASON}")
        await asyncio.sleep(3)
    try:
        os.remove("fedlist")
    except Exception as e:
...
async def _(event):
    ...
    for fed in fedList:
        ...
        await Ultra_bot.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await Ultra_bot.send_message(chat, f"/unfban {FBAN} {REASON}")
        await asyncio.sleep(3)
    try:
        os.remove("fedlist")
    except Exception as e:
...
async def _(event):
    ...
    if sysarg == "":
        ...
        async with Ultra.conversation(bot) as conv:
            try:
                ...
                if "Looks like" in audio.text:
                    ...
                    audio = await conv.get_response()
                    await Ultra.send_file(
                        event.chat_id,
                        audio,
                        caption=f"List of feds {user} has been banned in.\n\nCollected using Ultra.",
                        link_preview=False,
                    )
                else:
                    await Ultra.send_message(event.chat_id, audio.text)
                await Ultra.send_read_acknowledge(bot)
                await event.delete()
            except YouBlockedUserError:
                await ok.edit("**Error**\n `Unblock` @MissRose_Bot `and try again!")


@Ultra_cmd(pattern="fedinfo ?(.*)")
async def _(event):
