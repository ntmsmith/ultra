...
@Ultra_bot.on(events.NewMessage(outgoing=True))
@Ultra_bot.on(events.MessageEdited(outgoing=True))
    ...
    if "afk" not in current_message and "yes" in USER_AFK:
        try:
            if pic.endswith((".tgs", ".webp")):
                shite = await Ultra_bot.send_message(event.chat_id, file=pic)
                shites = await Ultra_bot.send_message(
                    event.chat_id,
                    get_string("afk_1").format(total_afk_time),
                )
            else:
                shite = await Ultra_bot.send_message(
                    event.chat_id,
                    get_string("afk_1").format(total_afk_time),
                    file=pic,
                )
        ...
        try:
            try:
                if pic.endswith((".tgs", ".webp")):
                    await Ultra_bot.send_message(LOG, file=pic)
...
async def _(event):
    ...
    if reply:
        ...
        pic = None
    if not USER_AFK:
        last_seen_status = await Ultra_bot(
        ...
        if reason:
            try:
                if pic.endswith((".tgs", ".webp")):
                    await Ultra_bot.send_message(event.chat_id, file=pic)
                    await Ultra_bot.send_message(
            ...
            try:
                if pic.endswith((".tgs", ".webp")):
                    await Ultra_bot.send_message(event.chat_id, file=pic)
                    await Ultra_bot.send_message(event.chat_id, get_string("afk_6"))
                else:
                    await Ultra_bot.send_message(
        ...
        try:
            if reason and pic:
                ...
                await Ultra_bot.send_message(LOG, get_string("afk_7").format(reason))
            elif pic:
                if pic.endswith((".tgs", ".webp")):
                    await Ultra_bot.send_message(LOG, file=pic)
                    await Ultra_bot.send_message(LOG, get_string("afk_8"))
                else:
                    await Ultra_bot.send_message(LOG, get_string("afk_8"), file=pic)
            else:
                await Ultra_bot.send_message(LOG, get_string("afk_8"))
        except BaseException:
            pass
