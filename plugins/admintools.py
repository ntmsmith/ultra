...
async def kck(ult):
    ...
    try:
        await Ultra_bot.kick_participant(ult.chat_id, user.id)
...
async def pin(msg):
    if not msg.is_private:
        ...
        await msg.get_chat()
    cht = await Ultra_bot.get_entity(msg.chat_id)
    ...
    if ch != "silent":
        ...
        try:
            await Ultra_bot.pin_message(msg.chat_id, xx, notify=slnt)
        except BadRequestError:
        ...
        try:
            await Ultra_bot.pin_message(msg.chat_id, xx, notify=False)
        except BadRequestError:
...
async def unp(ult):
    ...
    if msg and not ch:
        try:
            await Ultra_bot.unpin_message(ult.chat_id, msg)
        except BadRequestError:
        ...
        try:
            await Ultra_bot.unpin_message(ult.chat_id)
        except BadRequestError:
            return await xx.edit("`Hmm, I'm have no rights here...`")
        except Exception as e:
...
async def fastpurger(purg):
    ...
    async for msg in Ultra_bot.iter_messages(chat, min_id=purg.reply_to_msg_id):
        msgs.append(msg)
        count = count + 1
        msgs.append(purg.reply_to_msg_id)
        if len(msgs) == 100:
            await Ultra_bot.delete_messages(chat, msgs)
            msgs = []

    if msgs:
        await Ultra_bot.delete_messages(chat, msgs)
    done = await Ultra_bot.send_message(
        purg.chat_id,
...
async def fastpurgerme(purg):
    ...
    if not purg.reply_to_msg_id:
        return await eod(purg, "`Reply to a message to purge from.`", time=10)
    async for msg in Ultra_bot.iter_messages(
...
async def editer(edit):
    ...
    self_id = await Ultra_bot.get_peer_id("me")
    string = str(message[6:])
    i = 1
    async for message in Ultra_bot.iter_messages(chat, self_id):
        if i == 2:
