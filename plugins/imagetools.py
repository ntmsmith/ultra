...
async def sketch(e):
    ...
    cv2.imwrite("Ultra.png", pencil_sketch_IMG)
    await e.client.send_file(e.chat_id, file="Ultra.png")
    await xx.delete()
    os.remove(file)
    os.remove("Ultra.png")
...
async def ultd(event):
    ...
    if ultt.endswith(".tgs"):
        ...
        file = "ult.png"
    ult = cv2.imread(file)
    Ultra = cv2.cvtColor(ult, cv2.COLOR_BGR2GRAY)
    cv2.imwrite("ult.jpg", Ultra)
...
async def ultd(event):
    ...
    if ultt.endswith(".tgs"):
        ...
        file = "ult.png"
    ult = cv2.imread(file)
    Ultra = cv2.GaussianBlur(ult, (35, 35), 0)
    cv2.imwrite("ult.jpg", Ultra)
    await event.client.send_file(
        event.chat_id, "ult.jpg", force_document=False, reply_to=event.reply_to_msg_id
    )
...
async def ultd(event):
    ...
    Ultra = cv2.cvtColor(dan, cv2.COLOR_HSV2BGR)
    cv2.imwrite("ult.jpg", Ultra)
    await event.client.send_file(
        event.chat_id, "ult.jpg", force_document=False, reply_to=event.reply_to_msg_id
    )
