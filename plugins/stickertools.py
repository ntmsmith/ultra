...
async def waifu(animu):
    ...
    try:
        sticcers = await Ultra_bot.inline_query(
...
async def hehe(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            photo = io.BytesIO()
            photo = await Ultra_bot.download_media(message.photo, photo)
        elif "image" in message.media.document.mime_type.split("/"):
            await xx.edit(f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            await Ultra_bot.download_file(message.media.document, photo)
            if (
    ...
    if photo:
        ...
        pack = 1
        if len(splat) == 3:
            pack = splat[2]  # User sent Ultra_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with Ultra_bot.conversation("Stickers") as conv:
                await conv.send_message("/addsticker")
                await conv.get_response()
                await Ultra_bot.send_read_acknowledge(conv.chat_id)
...
async def ultdestroy(event):
    ...
    if not (
        ...
        return await eor(event, "`Reply to Animated Sticker only`")
    await event.client.download_media(ult, "Ultra.tgs")
    xx = await eor(event, "`Processing...`")
    os.system("lottie_convert.py Ultra.tgs json.json")
    ...
    os.system("lottie_convert.py json.json Ultra.tgs")
    await event.client.send_file(
        event.chat_id,
        file="Ultra.tgs",
...
async def ultiny(event):
    ...
    if not (reply and (reply.media)):
        ...
        return
    xx = await eor(event, "`processing...`")
    ik = await Ultra_bot.download_media(reply)
    im1 = Image.open("resources/extras/Ultra_blank.png")
    if ik.endswith(".tgs"):
