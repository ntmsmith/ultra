...
# Telegraph Things
telegraph = Telegraph()
telegraph.create_account(short_name="Ultra")
# ================================================================#
...
async def leave(ult):
    ...
    await Ultra_bot(LeaveChannelRequest(ult.chat_id))
...
async def _(event):
    result = await Ultra_bot(functions.channels.GetAdminedPublicChannelsRequest())
    output_str = ""
    r = result.chats
...
async def stats(
    ...
    unread = 0
    dialog: Dialog
    async for dialog in Ultra_bot.iter_dialogs():
    ...
    full_name = inline_mention(await Ultra_bot.get_me())
...
async def _(event):
    ...
    try:
        ok = await Ultra_bot.inline_query(Var.BOT_USERNAME, q)
...
async def rmbg(event):
    ...
    if event.reply_to_msg_id:
        ...
        dl = await Ultra_bot.download_media(reply)
    ...
    await Ultra_bot.send_file(event.chat_id, "ult.webp", reply_to=reply)
    os.remove(rmbgp)
    os.remove("ult.webp")
    await xx.delete()
...
async def telegraphcmd(event):
    ...
    if event.reply_to_msg_id:
        ...
        if getmsg.photo or getmsg.video or getmsg.gif:
            getit = await Ultra_bot.download_media(getmsg)
            try:
                variable = uf(getit)
                os.remove(getit)
